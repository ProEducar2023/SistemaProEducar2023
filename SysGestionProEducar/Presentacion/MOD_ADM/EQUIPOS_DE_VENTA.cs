...
{
    public partial class EQUIPOS_DE_VENTA : Form
    {
        string BOTON; DataTable dtEqv, dtTipoDoc, dtPrgNiv;
        ...
        int fila;
        public EQUIPOS_DE_VENTA()
        {
            InitializeComponent();
        }

        private void EQUIPOS_DE_VENTA_Load(object sender, EventArgs e)
        {
            KeyPreview = true;
            cargaGrid();
            cargaCombo();
        ...
        private void cargaComboNivel()
        {
            ...
            DataTable dt = nivBLL.obtenerNivelBLL();
        ...
        {
            dtEqv = eqvBLL.obtenerEquipoBLL();
            if (dtEqv.Rows.Count > 0)
            {
                foreach (DataRow rw in dtEqv.Rows)
                {
                    int rowId = dgw.Rows.Add();
                    DataGridViewRow row = dgw.Rows[rowId];
                    row.Cells["cod"].Value = rw["COD_EQVTA"].ToString();
        ...
        private void EQUIPOS_DE_VENTA_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        {
            //eq_vta_nivel_progBLL eqprniBLL = new eq_vta_nivel_progBLL();
            //eq_vta_nivel_progTo eqprniTo = new eq_vta_nivel_progTo();
            progNivelBLL prniBLL = new progNivelBLL();
            progNivelTo prniTo = new progNivelTo();
            prniTo.COD_PER = txt_cod.Text;
            DGW_DET.Rows.Clear();
            //eqprniTo.COD_EQVTA = dgw[0, dgw.CurrentRow.Index].Value.ToString();
            dtPrgNiv = prniBLL.obtenerProgramaNivelporCodigoBLL(prniTo);
            foreach (DataRow rw in dtPrgNiv.Rows)
            {
                ...
                row.Cells["numeracion"].Value = Convert.ToBoolean(rw["NUMERACION"]);
            }
        }
        ...
        private void BTN_GUARDAR2_Click(object sender, EventArgs e)
        {
            if (!validaProgNiv())
                return;
            DGW_DET.Rows.Add(txt_cod.Text, cboprograma.SelectedValue.ToString(), cboprograma.Text, cbonivel.SelectedValue.ToString(), cbonivel.Text, cbo_tipo_pla.SelectedValue.ToString(), ch_actpn.Checked, chk_numeracion.Checked);
        ...
        private bool validaProgNiv()
        {
            ...
            if (BTN_GUARDAR2.Visible)
            {
                var query = from DataGridViewRow row in DGW_DET.Rows
                            where (row.Cells["codp"].Value.ToString() == cboprograma.SelectedValue.ToString() && row.Cells["codni"].Value.ToString() == cbonivel.SelectedValue.ToString() && row.Cells["tip"].Value.ToString() == cbo_tipo_pla.SelectedValue.ToString())
                            select row;
                if (query.Count() > 0)
                {
        ...
        private void CARGAR_DETALLES1()
        {
            ...
            chk_numeracion.Checked = Convert.ToBoolean(DGW_DET.Rows[idx].Cells["numeracion"].Value);
        }
        ...
        private void BTN_MODIFICAR4_Click(object sender, EventArgs e)
        {
            ...
            DGW_DET.Rows[idx].Cells["numeracion"].Value = chk_numeracion.Checked;
            Panel1.Visible = false;
            DGW_DET.Visible = true;
        }
        ...
        private bool validaGuardarModificar()
        {
            ...
            if (btn_guardar.Visible == true)
            {
                ...
                if (query.Count() > 0)
                {
                    MessageBox.Show("Ya existe el vendedor !!!", "MENSAJE", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                    txt_cod.Focus();
                    return result = false;
                }
            }
        ...
        private void btn_guardar_Click(object sender, EventArgs e)
        {
            if (!validaGuardarModificar())
                return;
            DialogResult rs = MessageBox.Show("¿ Esta segura de adicionar una nueva persona para el EQ de Ventas ?", "MENSAJE", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
            if (rs == DialogResult.Yes)
            {
                ...
                if (!eqvBLL.adicionaEqVentasProgNivBLL(eqvTo, dtPrgNiv, ref errMensaje))
                    MessageBox.Show("ERROR............... !!!! \n" + errMensaje, "MENSAJE", MessageBoxButtons.OK, MessageBoxIcon.Error);
                else
                {
                    MessageBox.Show("Se adicionó correctamente a la Persona !!!", "Mensaje", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    //
                    dgw.Rows.Add(txt_cod.Text, cbo_tipo_doc.SelectedValue.ToString(), txt_nro_doc.Text, txt_desc.Text, txt_nom.Text, txt_pat.Text, txt_mat, txt_mail, ch_act.Checked);
        ...
        private void btn_modificar2_Click(object sender, EventArgs e)
        {
            ...
            if (rs == DialogResult.Yes)
            {
                ...
                eqvTo.ACTIVO = ch_act.Checked;
                dtPrgNiv = obtieneDTProgNiv();
                if (!eqvBLL.modificaActualizaEqVentasProgNivBLL(eqvTo, dtPrgNiv, ref errMensaje))
                    MessageBox.Show("ERROR............... !!!! \n" + errMensaje, "MENSAJE", MessageBoxButtons.OK, MessageBoxIcon.Error);
                else
                {
                    MessageBox.Show("SE MODIFICÓ CORRECTAMENTE LA PERSONA !!!" + errMensaje, "Mensaje", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    //
        ...
        private void btn_eliminar_Click(object sender, EventArgs e)
        {
            if (dgw.Rows.Count <= 0)
                return;
            DialogResult rs = MessageBox.Show("¿ Esta seguro de eliminar el Personal de Venta ?", "MENSAJE", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
            if (rs == DialogResult.Yes)
            {
                string errMensaje = "";
                ...
                if (!eqvBLL.eliminaEqVentaBLL(eqvTo, ref errMensaje))
                    MessageBox.Show("ERROR............... !!!! \n" + errMensaje, "MENSAJE", MessageBoxButtons.OK, MessageBoxIcon.Error);
                else
                {
                    MessageBox.Show("El Personal de Venta se eliminó correctamente", "MENSAJE", MessageBoxButtons.OK, MessageBoxIcon.Information);
