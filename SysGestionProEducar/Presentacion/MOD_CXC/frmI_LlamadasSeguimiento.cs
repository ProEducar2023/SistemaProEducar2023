...
{
    public partial class frmI_LlamadasSeguimiento : Form
    {
        ...
        DateTime? fecha_deposito; string nro_operacion; string institucion_bancaria; string cod_usu; string AÑO; string MES;
        ...
        public frmI_LlamadasSeguimiento(string nro_contrato, string cod_persona, string cod_gestor, string cod_motivo_llamada,
            DateTime fecha_operacion_llamada, DateTime? fecha_compromiso_pago, string observacion, decimal importe_pago,
            DateTime? fecha_deposito, string nro_operacion, string institucion_bancaria, string cod_usu, string AÑO, string MES)
        {
            InitializeComponent();
        ...
        private void frmI_LlamadasSeguimiento_Load(object sender, EventArgs e)
        {
            CARGAR_BANCO();
        ...
        {
            if (cod_motivo_llamada != "03")//
                habilita_controles(false);
            if (cod_motivo_llamada == "")
            {
            ...
            cbo_motivo.SelectedValue = cod_motivo_llamada;
            //dtp_fec_com_pago.Value = fecha_compromiso_pago;
            if (fecha_compromiso_pago == null)
                dtp_fec_com_pago.Value = fecha_operacion_llamada;
            else
                dtp_fec_com_pago.Value = fecha_compromiso_pago.Value;
            txt_observaciones.Text = observacion;
            txt_imp_pago.Text = importe_pago.ToString();
            if (fecha_deposito == null)
            {
                chk_fec_deposito.Checked = false;
                dtp_fec_deposito.Value = fecha_operacion_llamada;
            }
            else
            {
                if (cod_motivo_llamada == "03")
                {
                    chk_fec_deposito.Checked = true;
                    dtp_fec_deposito.Value = fecha_deposito.Value;
                }
            }
            txt_nro_operacion.Text = nro_operacion;
            if (institucion_bancaria != "")
                cbo_banco.SelectedValue = institucion_bancaria;
            else
        ...
        private void frmI_LlamadasSeguimiento_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void btn_grabar_Click(object sender, EventArgs e)
        {
            ...
            if (rs == DialogResult.Yes)
            {
                ...
                ddsTo.STATUS_CONFIRMACION_PAGO = null;
                ...
                if (!ddsBLL.adicionaLlamadasSeguimientoBLL(ddsTo, ref errMensaje))
        ...
        private bool valida()
        {
            ...
            if (cbo_motivo.SelectedValue.ToString() == "03")//Confirmacion de pago
            {
        ...
        private void chk_fec_deposito_CheckedChanged(object sender, EventArgs e)
        {
            if (chk_fec_deposito.Checked)
                dtp_fec_deposito.Enabled = true;
            else
                dtp_fec_deposito.Enabled = false;
        }
        ...
        private void cbo_motivo_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                if (cbo_motivo.SelectedValue.ToString() == "03")
        ...
        private void btn_modificar_Click(object sender, EventArgs e)
        {
            ...
            DialogResult rs = MessageBox.Show("¿ Está seguro de modificar este registro con el contrato " + nro_contrato + " ?", "MENSAJE", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
            if (rs == DialogResult.Yes)
            {
                string errMensaje = string.Empty;
                //int idx = dgw_llamadas.CurrentRow.Index;
                ...
                ddsTo.COD_USU = cod_usu;
                ddsTo.FEC_COD_USU = DateTime.Now;
                if (!ddsBLL.modificarLlamadasSeguimientoBLL(ddsTo, ref errMensaje))
                    MessageBox.Show("ERROR............... !!!! \n" + errMensaje, "MENSAJE", MessageBoxButtons.OK, MessageBoxIcon.Error);
                else
                {
                    MessageBox.Show("Se grabó correctamente !!!", "Mensaje", MessageBoxButtons.OK, MessageBoxIcon.Information);
