namespace Entidades
{
    public class contratoDetalleTo
    {
        public string COD_SUCURSAL { get; set; }
        public string NRO_DOC { get; set; }
        public string COD_PER { get; set; }
        public string COD_CLASE { get; set; }
        public string FE_AÑO { get; set; }
        public string FE_MES { get; set; }
        public string ITEM { get; set; }
        public string COD_ARTICULO { get; set; }
        public decimal CANTIDAD_PED { get; set; }
        public decimal CANTIDAD_ATEN { get; set; }
        public decimal CANTIDAD_ANUL { get; set; }
        public decimal PRECIO_UNIT { get; set; }
        public decimal VALOR_COMPRA { get; set; }
        public decimal POR_IGV { get; set; }
        public decimal POR_DSCTO { get; set; }
        public string STATUS_IGV { get; set; }
        public decimal VALOR_IGV { get; set; }
        public decimal VALOR_DSCTO { get; set; }
        public decimal AJUSTE_IGV { get; set; }
        public decimal AJUSTE_BI { get; set; }
        public string COD_D_H { get; set; }
        public string OBSERVACION { get; set; }
        public string NRO_PRESUPUESTO { get; set; }
        public string NRO_ITEM { get; set; }
        public string COD_CONDICION { get; set; }
        public string DESC_ARTICULO { get; set; }
        public string TIPO_PRECIO { get; set; }
        public string TIPO_ENTR_FAC { get; set; }
        public string NRO_FAC_PRE_UNI { get; set; }
        public string ST_VALOR_REFERENCIAL { get; set; }
        public string COD_KIT { get; set; }
    }
}
